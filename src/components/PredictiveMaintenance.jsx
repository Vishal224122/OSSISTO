...
export default function AnalyticsBiDashboards() {
  ...
  const capabilities = [
    {
      desc: "We support asset criticality and failure mode reviews, helping select the small number of machines where monitoring is genuinely worth the investment."
    },
    {
      desc: "We help install condition monitoring and set sensible alarm thresholds from baseline readings, which delivers most of the early value."
    },
    {
      desc: "We assist with linking alerts into your maintenance workflow, so a warning becomes a planned work order with the right parts reserved."
    },
    {
      desc: "We are explicit about data prerequisites, and where failure history is insufficient we begin structured logging rather than presenting untested prediction."
    }
  ];
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
          ...
          <div className="md:col-span-12 lg:col-span-12 space-y-5 text-center">
            ...
            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-black tracking-tight leading-tight max-w-2xl mx-auto">
              Maintenance that reduces real downtime
            </h2>
            <div className="space-y-4 text-slate-500 text-sm leading-relaxed max-w-3xl font-medium mx-auto text-justify">
              <p>
                Predictive maintenance works when it is applied selectively, with honest data and a maintenance team ready to act on it. Ossisto Global takes that practical route: find the assets where failure genuinely hurts, monitor them properly, and add prediction only where there is enough failure history to justify it.
              </p>
              <p>
                We begin with criticality rather than algorithms. A structured review looks at the consequence of failure, past downtime, spare availability and the ways each machine actually fails, and from that we select a small set of candidates. Monitoring is then designed around physical behaviour, using the measurements that reveal the relevant wear or fault for that kind of equipment.
              </p>
              <p>
                Condition monitoring with well-set thresholds delivers most of the early value, and we say so openly rather than overselling prediction. Where there is enough recorded failure history, we develop models and test them against events held back for the purpose. Alerts are fed into your maintenance workflow so a warning becomes a planned job with the right parts reserved. The aim is a maintenance function that moves step by step from reactive to condition-based, each step justified by measured results.
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="w-full md:w-3/5 p-8 lg:p-10 flex flex-col justify-between text-left">
              <div>
                ...
                <p className="text-slate-500 text-xs leading-relaxed font-medium max-w-md text-justify">
                  Unplanned downtime is expensive twice: lost production, plus the premium cost of emergency repair, expedited parts and overtime. Time-based maintenance mitigates this crudely, replacing healthy components on schedule while still missing failures that develop between intervals. Condition-based and predictive approaches address both problems, extending intervals on assets that are demonstrably healthy while catching degradation early on those that are not. For mid-sized manufacturers with limited maintenance headcount, the scheduling benefit is often larger than the avoided-failure benefit, because knowing which machine needs attention next week allows planned intervention during an existing changeover instead of a mid-shift breakdown. It also reduces spares inventory held purely as insurance, and captures failure knowledge in data rather than in the memory of a technician approaching retirement.
                </p>
              </div>
