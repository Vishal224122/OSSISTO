...
export default function QualityEngineeringTestAutomation() {
  ...
  const capabilities = [
    {
      desc: "We support quality and testing teams in deciding where automation is genuinely worth the effort, starting from what actually breaks rather than from what is easiest to automate."
    },
    {
      desc: "We help build and maintain automated checks that can be run repeatedly as systems change, with particular attention to keeping them reliable, since tests that fail unpredictably end up being ignored."
    },
    {
      desc: "We assist with the practical groundwork that testing depends on, including preparing and refreshing usable test data and organising how results and defects are reported back to your team."
    },
    {
      desc: "We can begin with one high-risk area and demonstrate a working automated run before extending further, and we aim to leave your own team able to maintain and extend what has been set up."
    }
  ];
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
          <div className="md:col-span-12 lg:col-span-12 space-y-5 text-center">
            ...
            <div className="space-y-4 text-slate-500 text-sm leading-relaxed max-w-3xl font-medium mx-auto text-justify">
              <p>
                When testing is manual, releases get postponed. Improvements queue up, and each release then carries more risk than the one before it. Ossisto Global builds the testing capability that breaks this cycle, so changes can be made in small, verified steps.
              </p>
              <p>
                We target automation where it compounds, starting from what actually breaks - defect history, frequently changed areas and business-critical paths - rather than automating the easiest cases first. Wherever possible we test at the interface level between systems, because those checks run faster and break less often than tests driven through screens.
              </p>
              <p>
                Test suites are engineered as proper software, with reusable components and stable references, since unreliable tests are worse than none because teams learn to ignore them. We also give explicit attention to test data, which is the most common practical blocker in enterprise and manufacturing applications. Results are published automatically with every build, so quality status is visible to your team rather than reported by us.
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="w-full md:w-3/5 p-8 lg:p-10 flex flex-col justify-between text-left">
              <div>
                ...
                <p className="text-slate-500 text-xs leading-relaxed font-medium max-w-md text-justify">
                  Release confidence determines how quickly a manufacturer can improve its own systems. Where regression testing takes a fortnight of manual effort, releases become quarterly events, changes batch up, and each deployment carries the combined risk of everything in it - which is precisely how ERP and MES changes cause production disruption. Automation inverts this into frequent small releases, each independently verified and easy to diagnose if something breaks. The direct savings on manual test effort are real but secondary to that operational effect. There is a specific manufacturing dimension too, because failures in ERP, MES or integration layers stop production rather than inconveniencing users, and validated regression coverage on those paths is the most practical protection available. Structured testing also produces the evidence customer and certification audits increasingly require.
                </p>
              </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
          <div className="md:col-span-5 space-y-4 text-left relative">
            <h2 className="text-2xl sm:text-3xl font-black text-black tracking-tight leading-tight max-w-sm relative z-10 pt-4">
              Our key areas for Quality Engineering
            </h2>
          </div>
