...
export default function ManufacturingAnalyticsOee() {
  ...
  const capabilities = [
    {
      desc: "We support agreeing consistent loss and OEE definitions per asset class, documented so every line and plant calculates the same measures."
    },
    {
      desc: "We help automate data capture from machines and existing systems, with simple downtime reason selection where operator input adds context sensors cannot."
    },
    {
      desc: "We assist with loss reporting and bottleneck analysis by cause, asset, product and shift, using reporting tools you already own."
    },
    {
      desc: "We prefer to start on one bottleneck asset with a measured baseline, and help establish the review routine that turns reports into action."
    }
  ];
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
          ...
          <div className="md:col-span-12 lg:col-span-12 space-y-5 text-center">
            ...
            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-black tracking-tight leading-tight max-w-2xl mx-auto">
              Trusted OEE, quantified losses and analytics
            </h2>
            <div className="space-y-4 text-slate-500 text-sm leading-relaxed max-w-3xl font-medium mx-auto text-justify">
              <p>
                Most plants believe they know their bottleneck. Measured properly, they are often wrong. Ossisto Global helps build analytics that show where availability, performance and quality losses actually occur, at machine, line and plant level, taken automatically from production and machine data rather than assembled in spreadsheets.
              </p>
              <p>
                The work begins with definitions, because inconsistent calculations are the most common reason plant analytics lose credibility. We agree what counts as planned and unplanned downtime, ideal cycle time, short stops and quality loss for each type of asset, and we write it down. Capture is then automated, with manual entry kept only where a person adds context a sensor cannot, such as the reason for a stoppage.
              </p>
              <p>
                Reporting is layered on purpose: live displays for operators, shift reviews for supervisors, weekly loss analysis for the plant, and comparable views across lines or sites for management. Beyond headline figures we look at the analyses that pay for themselves, such as changeover times, scrap by cause, where the bottleneck moves, and energy per unit. The aim is to move from describing what happened, to explaining why, to anticipating what will limit output next quarter.
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="w-full md:w-3/5 p-8 lg:p-10 flex flex-col justify-between text-left">
              <div>
                ...
                <p className="text-slate-500 text-xs leading-relaxed font-medium max-w-md text-justify">
                  Manufacturing improvement without measurement becomes a matter of opinion, and opinion tends to defend the status quo. Automated loss analysis makes the argument factual. It shows that a line believed to be capacity-constrained is actually changeover-constrained, or that the minor stops nobody logs cost more annually than the breakdowns everybody discusses. That clarity changes where capital and effort go, often deferring equipment purchases a throughput problem never required. Consistent measurement across lines and plants also enables genuine benchmarking and makes improvement gains verifiable rather than asserted. For manufacturers under margin and delivery pressure this is the highest-return digital investment available, because it typically unlocks existing capacity instead of adding cost. It also builds the labelled, contextualised dataset needed later for predictive maintenance and AI-based process optimisation.
                </p>
              </div>
