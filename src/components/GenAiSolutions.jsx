...
export default function GenAiSolutions() {
  ...
  const capabilities = [
    {
      desc: "We screen candidate use cases against three conditions - the knowledge is retrievable, the task is slow and repetitive, and a wrong answer is recoverable - which deliberately eliminates most proposals."
    },
    {
      desc: "We build retrieval-grounded assistants over your own manuals, specifications, procedures, quality records and engineering archives, citing source documents so users can verify rather than trust."
    },
    {
      desc: "We treat document preparation as the main workstream, structuring, tagging and digitising scanned legacy content at predictable unit cost before any model work begins."
    },
    {
      desc: "We agree an accuracy threshold and a labelled evaluation set before building, then score against it and regression-test on every change, with an explicit option to stop."
    },
    {
      desc: "We deploy inside your own cloud boundary, with access controls mirroring existing document permissions, audit logging and defined limits on what an assistant may not answer."
    },
    {
      desc: "Pilots run fixed-scope and fixed-price over six to eight weeks, with specialists placed onsite in Europe and the US where security review or local accountability is required."
    }
  ];
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
          <div className="md:col-span-12 lg:col-span-12 space-y-5 text-center">
            ...
            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-black tracking-tight leading-tight max-w-2xl mx-auto">
              Grounded assistants over your own documents - Gen AI scoped to measurable payback
            </h2>
            <div className="space-y-4 text-slate-500 text-sm leading-relaxed max-w-3xl font-medium mx-auto text-justify">
              <p>
                Manuals, quality records, engineering reports, specifications, and supplier documents. Ossisto Global builds secure, retrieval-grounded AI assistants that work only with your approved content, providing source-backed answers users can verify instead of relying on unsupported AI responses.
              </p>
              <p>
                Every solution begins with a careful evaluation to identify use cases where information is available, tasks are repetitive and time-consuming, and any incorrect answer can be reviewed and corrected. We create measurable benchmarks before development, so performance is tested rather than assumed. Each pilot is intentionally small, focused on clear business outcomes such as time saved and accuracy achieved, with the flexibility to scale successful solutions or stop those that do not meet expectations.
              </p>
              <p>
                Governance is built into every deployment through role-based access, audit trails, and clear boundaries on what the assistant can and cannot answer. We also recognise that organising and structuring enterprise content is a key part of success and plan for it from the start. The result is practical, production-ready AI that delivers measurable business value—not strategy documents or impressive demonstrations, but reliable tools built around your own knowledge.
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="w-full md:w-3/5 p-8 lg:p-10 flex flex-col justify-between text-left">
              <div>
                <h3 className="text-black text-xl md:text-2xl font-black leading-snug mb-4 max-w-md">
                  Immediate benefits of Gen AI use cases
                </h3>
                <p className="text-slate-500 text-xs leading-relaxed font-medium max-w-md text-justify">
                  Manufacturing runs on documented knowledge that is genuinely hard to find. A service technician searching six hundred pages of manuals, an engineer looking for how a similar problem was solved in 2016, a quality lead assembling evidence for a customer audit - each spends significant time on retrieval rather than judgement. Grounded generative AI compresses that retrieval time substantially, and the benefit scales with the size of your document estate rather than requiring new data collection. It also addresses workforce transition directly, since an assistant built on the documented output of retiring staff preserves part of that knowledge in accessible form. The important discipline is scope. Value comes from narrow, well-grounded assistants with verifiable answers, not from general-purpose chat disconnected from your actual records.                </p>
              </div>
            </div>
